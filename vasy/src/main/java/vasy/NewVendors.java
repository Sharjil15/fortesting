	...
	public static void main(String[] args) throws InterruptedException {
	 /* System.setProperty("webdriver.chrome.driver", "C:\\Users\\Sharjil\\eclipse-workspace\\drivers\\chromedriver_win32\\chromedriver.exe");
		...
		*/
		...
		Thread.sleep(5000);
		WebElement Vendor=driver.findElement(By.partialLinkText("Vendors"));
		Vendor.click();

		WebElement createNewvendor=driver.findElement(By.xpath("//a[@id='newContact']"));
		createNewvendor.click();

		WebElement firstName=driver.findElement(By.name("firstName"));
		firstName.sendKeys("VendorFirstName");

		WebElement  lastName=driver.findElement(By.name("lastName"));
		lastName.sendKeys("VendorLastName");

		WebElement  CompName=driver.findElement(By.id("companyName"));
		CompName.sendKeys("Vendor Company Name");

		WebElement  mobNumber=driver.findElement(By.id("mobNo"));
		mobNumber.sendKeys("987654356666");

		WebElement  watsNumber=driver.findElement(By.name("telephone"));
		watsNumber.sendKeys("98765432111");
		Thread.sleep(5000);

		WebElement email=driver.findElement(By.name("email"));
		email.sendKeys("Vendor@gmail.com");

		WebElement remark=driver.findElement(By.name("remark"));
		remark.sendKeys("I am Vendor");
		...
		for(int i=0;i<statelist.size();i++) {
			...
			if(statelist.get(i).getText().contains("Input Service Distributor")) {
		...
		driver.findElement(By.id("contactAddressVos[0].addressLine1")).sendKeys("Vendors Colony,Vendor road");
		...
		for(int i=0;i<statelist1.size();i++) {
			...
			if(statelist1.get(i).getText().contains("Haryana")) {
		...
		for(int i=0;i<citylist.size();i++) {
			...
			if(citylist.get(i).getText().contains("Babiyal")) {
				citylist.get(i).click();
				break;
			}
		}
		driver.findElement(By.xpath("//input[@id='contactAddressVos[0].pinCode']")).sendKeys("457860");
		...
		for(int i=0;i<selectpay.size();i++) {
			...
			if(selectpay.get(i).getText().contains("phone pay12")) {
		...
		for(int i=0;i<paytermslist.size();i++) {
			...
			if(paytermslist.get(i).getText().contains("r_demo")) {
