...
public class NewAgent {
	...
	public void TestAgent () throws Exception { 
		...
		Thread.sleep(5000);
		WebElement Agent=driver.findElement(By.partialLinkText("Agent"));
		Agent.click();
		...
		driver.findElement(By.id("contactAddressVos[0].addressLine1")).sendKeys("Agent Colony,Transport road");
		driver.findElement(By.name("contactAddressVos[0].addressLine2")).sendKeys("Agent City,near Agent pvt");	
		...
		for(int i=0;i<statelist1.size();i++) {
			...
			if(statelist1.get(i).getText().contains("Uttar Pradesh")) {
		...
		for(int i=0;i<citylist.size();i++) {
			...
			if(citylist.get(i).getText().contains("Agra")) {
		...
		driver.manage().timeouts().implicitlyWait(30,TimeUnit.SECONDS);
		driver.findElement(By.xpath("//input[@name='commission']")).sendKeys("5000");
		...
		String ExpectedTitle = "AgentFirstName AgentLastName | Agent";
